...
{h:"What an Agent Actually Is — and When You Don't Need One",b:`Before building anything, it helps to understand exactly what an agent is and, just as importantly, when a simpler approach would serve you better.

Every agent runs the same fundamental loop: **Thought** (reason about the current state and decide what to do next), **Action** (call a tool via structured output, then stop generating), **Observation** (receive the tool's results back into context, and repeat) [26]. Three action formats exist — JSON, Code, and Function-calling — and the industry has converged on **Code Agents**, because LLMs tend to write better code than synthetic tool-call tokens [14][26]. But regardless of format, the loop is the same: think, act, observe, repeat until done.

The crucial insight, though, is that most tasks don't need this loop at all. As one widely-cited guideline puts it: "Find the simplest solution possible." [1] **Workflows** — predefined code paths where you control the sequence — beat agents whenever the steps are predictable. Five composable workflow patterns cover a remarkable amount of ground [1][29]: Prompt Chaining (output of one step feeds the next), Routing (classify input, then dispatch to the right handler), Parallelization (fan out and merge), Orchestrator-Workers (a coordinator dispatches subtasks), and Evaluator-Optimizer (generate-then-critique in a loop). Think of these as atoms; every agent harness is a molecule assembled from them.

The decision to reach for an agent should be deliberate. If you can draw the flowchart before you run it, you have a workflow — and workflows are faster, cheaper, and far easier to debug. Agents earn their complexity only when the task genuinely requires adaptive, multi-step reasoning that you cannot predetermine.`},
...
<span style={{fontSize:10,letterSpacing:"2.5px",textTransform:"uppercase",color:"#fff",background:"#111",padding:"4px 12px",borderRadius:3,fontFamily:"system-ui,sans-serif",fontWeight:700}}>V10</span>
