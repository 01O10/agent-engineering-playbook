...
function generateObsidianVault(chapters,matrix,phases,meta,R){
const refs={};R.forEach(r=>{refs[r.id]={t:r.s,u:r.u}});
const toMd=(t)=>t.replace(/\[(\d+)\]/g,(_,id)=>refs[id]?'[^'+id+']':'['+id+']');
const mermaid={"agency-spectrum":'graph LR\n    A["Deterministic<br/>Fixed sequence"] --> B["Agentic Workflow<br/>LLM decides at key points"] --> C["Autonomous Agent<br/>Self-directed loop"]\n    style A fill:#dae8fc\n    style B fill:#e1d5e7\n    style C fill:#fce4d6',
"action-space":'graph TB\n    L1["Layer 1 — Bound Tools, under 20<br/>bash, filesystem, code_exec"] --> L2["Layer 2 — The Computer<br/>APIs, databases, packages via code"]\n    style L1 fill:#d5e8d4\n    style L2 fill:#e8edff',
"prompt-layout":'graph TB\n    A["System Prompt — static"] --> B["Tool Definitions — stable"] --> C["Project Config — per-project"] --> D["Conversation — grows"]\n    style A fill:#fce4d6\n    style B fill:#fff2cc\n    style C fill:#e1d5e7\n    style D fill:#dae8fc',
"eval-flywheel":'graph LR\n    A["Traces"] --> B["Annotation"] --> C["Taxonomy"] --> D["Dataset"] --> E["Fixes"] --> A\n    style A fill:#fce4d6\n    style B fill:#e1d5e7\n    style C fill:#fff2cc\n    style D fill:#d5e8d4\n    style E fill:#dae8fc',
"init-coder":'graph LR\n    I["Initializer<br/>Plans + progress file"] --> P["Progress File"] --> C["Coder<br/>One feature at a time"]\n    C -.->|updates| P\n    style I fill:#e8edff\n    style P fill:#fff2cc\n    style C fill:#d5e8d4',
"hitl-spectrum":'graph LR\n    A["Pre-execution<br/>SAFEST"] --> B["Checkpoint"] --> C["Post-execution"] --> D["Exception-based<br/>FASTEST"]\n    style A fill:#fce4d6\n    style D fill:#d5e8d4'};
const vaL={L:"Predictable",M:"Variable",H:"Highly variable"};
const rr={arch:"Agent Architect",be:"Harness Engineer",ml:"Prompt & Eval Eng",devops:"Agent Ops Eng",pm:"Agent Product Mgr",team:"Full Team",infra:"Model Ops Eng",ux:"Agent UX Designer",domain:"Domain Expert"};
let out="";
// Index
out+="---\ntags: [agent-engineering, MOC]\n---\n\n# Agent Engineering Playbook\n\n## Guide\n\n";
chapters.forEach((c,i)=>{out+=(i+1)+". [["+String(i+1).padStart(2,"0")+" - "+c.tab.replace("→","to")+"]]\n"});
out+="\n## Planner\n\n- [[Decision Matrix]]\n";
phases.forEach((p,i)=>{out+="- [[Phase "+(i+1)+" - "+p.name.split("—")[0].trim().replace("Phase "+(i+1)+" ","")+"]]\n"});
out+="\n- [[References]]\n\n";
out+="---FILE:00 - Index.md---\n";
// Chapters
chapters.forEach((c,ci)=>{
out+="---FILE:"+String(ci+1).padStart(2,"0")+" - "+c.tab.replace("→","to")+".md---\n";
out+="---\ntags: [agent-engineering]\nchapter: "+String(ci+1).padStart(2,"0")+"\n---\n\n# "+c.title+"\n\n---\n\n";
c.sections.forEach(s=>{
out+="## "+s.h+"\n\n";
if(s.diagram&&mermaid[s.diagram])out+="```mermaid\n"+mermaid[s.diagram]+"\n```\n\n";
out+=toMd(s.b)+"\n\n";
if(s.takeaways&&s.takeaways.length){out+="> [!summary] Key Takeaways\n";s.takeaways.forEach((t,i)=>{out+="> "+(i+1)+". "+t+"\n"});out+="\n"}
});
out+="\n---\n\n## References\n\n";
const used=new Set();c.sections.forEach(s=>{[...s.b.matchAll(/\[(\d+)\]/g)].forEach(m=>used.add(m[1]))});
[...used].sort((a,b)=>a-b).forEach(id=>{const r=refs[id];if(r)out+="[^"+id+"]: ["+r.t+"]("+r.u+")\n"});
out+="\n";
});
// Decision Matrix
out+="---FILE:Planner/Decision Matrix.md---\n";
out+="# Decision Matrix\n\n| Dimension | Key Questions | What to Document |\n|---|---|---|\n";
matrix.forEach(r=>{out+="| **"+r.d+"** | "+r.q+" | "+r.doc+" |\n"});
out+="\n";
// Phases
phases.forEach((p,pi)=>{
out+="---FILE:Planner/Phase "+(pi+1)+" - "+p.name.split("—")[0].trim().replace("Phase "+(pi+1)+" ","")+".md---\n";
out+="# "+p.name+"\n\n"+p.desc+"\n\n---\n\n";
p.tasks.forEach(t=>{
const m=meta[t.id]||{cx:"?",va:"?",ppl:"?",who:"?",skills:"?",scope:"?"};
out+="## "+t.id+": "+t.task+"\n\n";
out+="- **Complexity:** "+m.cx+"/5 · **Variability:** "+(vaL[m.va]||m.va)+"\n";
out+="- **Participants:** "+m.ppl+" — "+m.who+"\n";
out+="- **Skills:** "+m.skills+"\n";
if(t.deps.length)out+="- **Dependencies:** "+t.deps.join(", ")+"\n";
out+="- **Scope drivers:** "+m.scope+"\n\n";
out+=t.detail+"\n\n";
out+="> [!check] Done when\n> "+t.done+"\n\n";
out+="**Deliverable:** "+t.deliverable+"\n\n---\n\n";
});
});
// References
out+="---FILE:References.md---\n# References\n\n";
R.forEach(r=>{out+="- **["+r.id+"]** ["+r.s+"]("+r.u+")\n"});
return out;
}
...
{h:"The Paradigm Shift: It's the Harness, Not the Model",b:`Assuming you've determined where on the agency spectrum your system should sit, the next thing to internalize is where the real engineering challenge lies — and it is probably not where you expect.
...
}} style={{padding:"8px 14px",borderRadius:6,border:"1px solid #555",background:"#555",color:"#fff",fontSize:11,fontWeight:600,cursor:"pointer",fontFamily:"system-ui,sans-serif"}}>↓ Export</button>
<div id="export-menu" style={{display:"none",position:"absolute",top:"100%",left:0,marginTop:4,background:"#fff",border:"1px solid #ddd",borderRadius:8,boxShadow:"0 4px 12px rgba(0,0,0,0.1)",zIndex:10,minWidth:220,fontFamily:"system-ui,sans-serif"}}>
<div style={{padding:"8px 12px",fontSize:10,fontWeight:700,color:"#999",textTransform:"uppercase",letterSpacing:"0.5px",borderBottom:"1px solid #f0f0f0"}}>Copy to clipboard</div>
{[
{label:"Obsidian Vault (all files)",desc:"16 .md files with Mermaid diagrams",fn:()=>generateObsidianVault(chapters,matrix,phases,meta,R)},
{label:"Skill: Architecture Advisor",desc:"Decision matrix + design principles",fn:()=>SKILLS.advisor},
{label:"Skill: Project Planner",desc:"56-task phased backlog template",fn:()=>SKILLS.planner},
].map((item,i)=>(
<div key={i} onClick={()=>{try{navigator.clipboard.writeText(item.fn());setCopyMsg(item.label);setTimeout(()=>setCopyMsg(""),2000)}catch(e){}document.getElementById("export-menu").style.display="none"}} style={{padding:"10px 12px",cursor:"pointer",borderBottom:i<2?"1px solid #f5f5f5":"none",transition:"background 0.1s"}} onMouseOver={e=>e.currentTarget.style.background="#f8f8f8"} onMouseOut={e=>e.currentTarget.style.background="transparent"}>
<div style={{fontSize:12,fontWeight:600,color:"#333"}}>{item.label}</div>
<div style={{fontSize:10,color:"#999",marginTop:2}}>{item.desc}</div>
</div>
))}
</div>
</div>
{copyMsg&&<span style={{fontSize:11,color:"#2d6a2e",fontWeight:600,fontFamily:"system-ui,sans-serif",padding:"8px 0"}}>✓ Copied: {copyMsg}</span>}
</div></div>
