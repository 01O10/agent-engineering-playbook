...
**Anthropic's own guidance is worth heeding** [1]: "frameworks make it easy to get started by simplifying standard low-level tasks. However, they often create extra layers of abstraction that can obscure the underlying prompts and responses, making them harder to debug. They can also make it tempting to add complexity when a simpler setup would suffice. We suggest that developers start by using LLM APIs directly." Many patterns in this lesson can be implemented in a few lines of code without any framework.

**The practical decision tree:** Quick idea validation or MVP → CrewAI or OpenAI Agents SDK for speed. Complex stateful workflows, long-running tasks, production reliability → LangGraph. Type safety and code quality priority → Pydantic AI. Open-weights or edge deployment → smolagents. AWS-native stack → Strands. Full control, no abstraction overhead → raw API with the patterns from this lesson. In all cases, if you use a framework, make sure you understand the underlying code — incorrect assumptions about what's happening under the hood are a common source of bugs [1].`}
]},
...
{h:"Human-in-the-Loop (HITL)",b:`The lesson so far treats human intervention as binary: the agent runs or blocks for confirmation. In reality, HITL is a spectrum with its own design discipline.
...
<h1 style={{fontSize:27,fontWeight:800,lineHeight:1.12,margin:"0 0 8px",letterSpacing:"-0.6px"}}>Agent Engineering: The Complete Lesson</h1>
...
<button onClick={()=>setView("lesson")} style={{padding:"8px 18px",borderRadius:6,border:view==="lesson"?"2px solid #1a1a1a":"1px solid #ddd",background:view==="lesson"?"#1a1a1a":"#fff",color:view==="lesson"?"#fff":"#666",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"system-ui,sans-serif"}}>Lesson</button>
